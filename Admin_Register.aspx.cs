              ...
              using (SqlCommand cmd = new SqlCommand(query, con))
              {
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                  if (reader.HasRows)
                  {
                    ...
                    string prquery = @"Insert Into Member_Payment(User_Id, Order_Id, Amount, IsComplete) Output Inserted.Id Values(@Uid, @Oid, @price, 0)";
                    string[] prparamN = { "@Uid", "@Oid", "@price" };
                    string[] prparamV = { Session["Register_id"].ToString(), Session["Order_No"].ToString(), Session["price"].ToString() };
